...
void implode(boolean smooth, boolean random) {
  for (int x = 0; x < srcImg.width; x++) {
    for (int y = 0; y < srcImg.height; y++) {
      ...
      if (x % spacing == 0 && y % spacing == 0  && alpha(srcImg.get(x, y)) > 5) {
        float xx = random(-6*width, 6*width);
        float yy = random(-6*height, 6*height);
  ...
  for (int i = 0; i < pixelGrid.size(); i++) {
    pixelGrid.get(i).setDestination(pixelPos.get(i).x, pixelPos.get(i).y);
  }
}
...
void brush(float x, float y, int size, int intensity, boolean type) { // size = 300
  for (int i = 0; i < pixelGrid.size(); i++) {
    ...
    if (d <= size) {
      ...
      pixelGrid.get(pixelGrid.size()-1).setDestination(newX, newY); // true, true
